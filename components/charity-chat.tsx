  ...
  return (
    <div className="flex-1 flex flex-col bg-white min-h-0">
      ...
      <div className="h-12 bg-gradient-to-b from-[#f8f8f8] to-[#f0f0f0] border-b border-black/5 flex items-center justify-center px-4 shrink-0">
        <span className="text-[13px] font-semibold text-black/80">Charity</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 p-4 space-y-3">
        {messages.map((message) => (
          <div key={message.id} className="flex flex-col relative">
            ...
            >
              <div className={`rounded-2xl px-4 py-2 cursor-pointer ${message.role === 'assistant' ? 'bg-[#e9e9eb] text-black' : 'bg-[#0b84fe] text-white'}`}>
                              ...
                              if (line.startsWith('BUTTON:')) {
                                ...
                                return (
                                  <button
                                    ...
                                    className="block mt-2 px-4 py-2 bg-[#0b84fe] text-white rounded-lg hover:brightness-110 transition-all text-sm"
                                  >
                                    {buttonText}
                                  </button>
                                )
            ...
            <div className="max-w-[75%] self-start">
              <div className="rounded-2xl px-4 py-3 bg-[#e9e9eb]">
                <div className="flex gap-1.5 items-center">
                  <span className="w-2 h-2 rounded-full bg-gray-500/70 animate-[bounce_1s_ease-in-out_infinite]" />
                  <span className="w-2 h-2 rounded-full bg-gray-500/70 animate-[bounce_1s_ease-in-out_infinite_0.2s]" />
                  <span className="w-2 h-2 rounded-full bg-gray-500/70 animate-[bounce_1s_ease-in-out_infinite_0.4s]" />
                </div>
              </div>
            </div>
          </div>
      ...
      <form onSubmit={handleSubmit} className="p-3 border-t border-black/5 shrink-0">
        <div className="bg-white border border-black/10 rounded-full px-4 py-2 flex items-center gap-2">
          <input
            type="text"
          ...
          <button
            ...
            className="w-6 h-6 rounded-full bg-[#0b84fe] flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed hover:brightness-110 transition-all"
          >
