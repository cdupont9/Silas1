...
import { Plane, MapPin, Calendar, Users, Wallet, Bot, ChevronLeft, CheckCircle, Plus, Search, ArrowRight, Sparkles, MessageSquare, Shield, X, Map, CreditCard, Share, Trash2, Armchair, Info, Mail, MessageCircle, User, Check, Heart, Ban, Loader2, FileText, Settings, Award, Edit3, Utensils, Baby } from 'lucide-react';
...
interface Traveler {
    ...
    seatPref: 'Window' | 'Aisle' | 'Middle' | 'Any';
    mealPref: 'Standard' | 'Vegetarian' | 'Gluten Free' | 'Kids Meal';
    freqFlyer?: string;
    dob?: string;
}
...
const INITIAL_TRAVELERS: Traveler[] = [
    { id: 'eloise', name: 'Eloise Sawyer', role: 'Adult', seatPref: 'Aisle', mealPref: 'Standard', freqFlyer: 'LF-882910' },
    { id: 'daniel', name: 'Daniel Sawyer', role: 'Adult', seatPref: 'Window', mealPref: 'Standard', freqFlyer: 'LF-992102' },
    { id: 'talulah', name: 'Talulah Sawyer', role: 'Child', seatPref: 'Window', mealPref: 'Kids Meal', dob: '05/12/2015' },
    { id: 'rue', name: 'Rue Sawyer', role: 'Child', seatPref: 'Middle', mealPref: 'Kids Meal', dob: '08/02/2018' },
];
...
export const CharityFlyApp: React.FC<CharityFlyAppProps> = ({ onClose, onShareContext }) => {
  ...
  // Profile & Points State
  const [travelers, setTravelers] = useState<Traveler[]>(INITIAL_TRAVELERS);
  const [pointsBalance, setPointsBalance] = useState(142050);
  ...
  const handleChangeSeats = () => {
      ...
      // We pass the current passengers to regenerate flights with correct insights
      const mockFlight = generateFlights('JFK', 'SJO', activeTripForManage?.passengers || ['Eloise'])[0]; 
  ...
  const handleUpdateTraveler = (id: string, updates: Partial<Traveler>) => {
  ...
  const generateFlights = (from: string, to: string, currentPassengers: string[]): Flight[] => {
      // Context Analysis
      const isFamily = currentPassengers.some(p => ['Talulah', 'Rue'].includes(p));
      const isSolo = currentPassengers.length === 1;

      return [
        { 
            id: 'f1', airline: 'Lifetime Air', flightNum: 'LF-101', dep: from || 'JFK', arr: to || 'LAS', depTime: '08:00 AM', arrTime: '11:30 AM', duration: '3h 30m', price: 245, stops: 0, 
            insight: isFamily 
                ? "Best option for families. Direct flight minimizes stress for kids."
                : isSolo
                    ? "Quickest route. Ideal for maximizing your solo trip time."
                    : "Best direct option. Great start to the trip.",
            isBest: true 
        },
        { 
            ...
            insight: isFamily
                ? "Cheaper, but the layover might risk missing the connection with toddlers."
                : "Economical choice if you don't mind a brief stop."
        },
        { 
            id: 'f3', airline: 'Lifetime Air', flightNum: 'LF-404', dep: from || 'JFK', arr: to || 'LAS', depTime: '04:00 PM', arrTime: '07:30 PM', duration: '3h 30m', price: 280, stops: 0,
            insight: isFamily
                ? "Good flight, but arrives late. The kids might be tired."
                : isSolo
                    ? "Allows you to work a half-day before heading out."
                    : "Arrives just in time for a late dinner."
        },
        { 
            ...
            insight: isFamily
                ? "Premium cabin available. Maximum comfort for the whole family."
                : "Premium cabin available. Treat yourself to extra comfort."
        },
    ];
  };

  const handleSearch = () => {
      setSearchResults(generateFlights(origin, destination, passengers));
  ...
  const handleSeatClick = async (seat: Seat) => {
      ...
      const currentPassengerName = passengers[activePassengerIdx];
      // Find the traveler profile for preferences
      const traveler = travelers.find(t => t.name.includes(currentPassengerName)) || travelers[0];

      // Check if already selected by same person to toggle off
      const currentAssignmentEntry = Object.entries(seatAssignments).find(([p, s]) => (s as Seat).id === seat.id);
      if (currentAssignmentEntry && currentAssignmentEntry[0] === currentPassengerName) {
          const newAssignments = { ...seatAssignments };
          delete newAssignments[currentPassengerName];
      ...
      setSeatInsightPopup({
          seat,
          passenger: currentPassengerName,
          loading: true
      });
      ...
      // Determine Trip Context for AI
      const isFamilyTrip = passengers.some(p => ['Talulah', 'Rue'].includes(p));
      const isSoloTrip = passengers.length === 1;
      const tripType = isSoloTrip ? "SOLO TRIP" : isFamilyTrip ? "FAMILY TRIP" : "GROUP TRIP";

      const prompt = `
          You are Silas, a travel AI assistant.
          Passenger: ${currentPassengerName} (${traveler.role}).
          Trip Type: ${tripType}.
          Preferences: Seat=${traveler.seatPref}, Meal=${traveler.mealPref}.
          
          Selected Seat: ${seat.id} (Row ${seat.row}, ${seatType}).
          Current Assignments: ${otherSeats || 'None yet'}.
          Exit Row: ${isExitRow}.
          
          Analyze if this is a good seat choice based on their profile and the trip type.
          - If SOLO: Prioritize their specific seat preference and comfort. Exit row is great for legroom. Do not mention family/kids.
          - If FAMILY: Child must sit near parent. No Exit Rows for kids.
          - If Preference Mismatch: Warn them (e.g. they like Window but picked Aisle).
          
          Start response with a verdict in CAPS: "GOOD CHOICE", "WARNING", "NOTE", or "PERFECT".
          Then provide a 1-sentence explanation directly addressing the passenger's needs.
      `;
      ...
      try {
          ...
          setSeatInsightPopup(prev => prev ? { 
              ...prev, 
              loading: false, 
              analysis: "NOTE: Unable to connect to Silas server. Proceed with caution." 
          } : null);
      }
  };
  ...
  const confirmSeatSelection = () => {
      ...
      // Clear seat if taken by someone else locally
      const existingEntry = Object.entries(newAssignments).find(([_, s]) => (s as Seat).id === seat.id);
      if (existingEntry) {
  ...
  const handleBookingComplete = () => {
      if (selectedFlight) {
          ...
          const newTrip: Trip = {
              ...
              silasNote: `Booked via Lifetime Flights. ${selectedFlight.insight}`,
              coPilot: true,
          ...
          // Add Points
          const ptsEarned = Math.floor(newTrip.price! * 5);
  ...
  // 3. EDIT TRAVELER SHEET
  const renderEditTraveler = () => {
      const traveler = travelers.find(t => t.id === editingTravelerId);
      ...
      return (
          ...
          >
              <motion.div 
                  initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
                  className="w-full bg-white rounded-t-[2rem] p-6 pb-12"
                  onClick={e => e.stopPropagation()}
              >
                  <div className="flex justify-between items-center mb-6">
                      <div className="flex items-center gap-3">
                          ...
                          <div>
                              <h3 className="font-bold text-xl text-slate-900">{traveler.name}</h3>
                              <p className="text-sm text-gray-500">{traveler.role}</p>
                          </div>
                      </div>
                      <button onClick={() => setEditingTravelerId(null)} className="p-2 bg-gray-100 rounded-full"><X size={20} /></button>
                  </div>

                  <div className="space-y-6">
                      {/* Seat Pref */}
                      <div>
                          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-3">Seat Preference</label>
                          <div className="grid grid-cols-4 gap-2">
                              {['Window', 'Aisle', 'Middle', 'Any'].map(opt => (
                                  <button
                                      key={opt}
                                      onClick={() => handleUpdateTraveler(traveler.id, { seatPref: opt as any })}
                                      className={`py-2 rounded-lg text-xs font-bold border transition-all ${traveler.seatPref === opt ? 'bg-sky-600 text-white border-sky-600' : 'bg-white text-gray-600 border-gray-200'}`}
                                  >
                                      {opt}
                                  </button>
                              ))}
                      ...
                      {/* Meal Pref */}
                      <div>
                          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-3">Meal Preference</label>
                          <div className="grid grid-cols-2 gap-2">
                              {['Standard', 'Vegetarian', 'Gluten Free', 'Kids Meal'].map(opt => (
                                  <button
                                      key={opt}
                                      onClick={() => handleUpdateTraveler(traveler.id, { mealPref: opt as any })}
                                      className={`py-2 px-3 rounded-lg text-xs font-bold border text-left transition-all flex items-center gap-2 ${traveler.mealPref === opt ? 'bg-green-50 text-green-700 border-green-200' : 'bg-white text-gray-600 border-gray-200'}`}
                                  >
                                      {opt === 'Standard' && <Utensils size={12} />}
                                      {opt === 'Kids Meal' && <Baby size={12} />}
                                      {opt}
                                  </button>
                              ))}
                          </div>
                      ...
                      {traveler.role === 'Adult' && (
                          <div className="bg-gray-50 p-3 rounded-xl border border-gray-200 flex justify-between items-center">
                              <span className="text-sm font-medium text-gray-600">Frequent Flyer #</span>
                              <span className="font-mono text-sm font-bold text-slate-900">{traveler.freqFlyer || 'Add'}</span>
                          </div>
                      )}
                  </div>

                  <button onClick={() => { setEditingTravelerId(null); showToast("Preferences Updated"); }} className="w-full mt-8 py-3 bg-slate-900 text-white font-bold rounded-xl">
                      Save Changes
                  </button>
              </motion.div>
          </motion.div>
  ...
  const renderProfile = () => (
      <div className="flex-1 overflow-y-auto pb-24">
          <div className="relative bg-slate-900 text-white p-6 pb-24 overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-sky-500/20 rounded-full blur-[80px] pointer-events-none"></div>
              
              <div className="flex justify-between items-start mb-8 relative z-10">
                  <div>
                      <h2 className="text-3xl font-bold">Eloise Sawyer</h2>
                      <p className="text-sky-300 font-medium">Lifetime Platinum Member</p>
                  </div>
              ...
              {/* Loyalty Card */}
              <div className="relative z-10 w-full aspect-[1.6] rounded-2xl p-6 bg-gradient-to-br from-sky-600 via-blue-700 to-indigo-900 shadow-2xl border border-white/10 flex flex-col justify-between overflow-hidden">
                  <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10"></div>
                  <div className="flex justify-between items-start">
                      <div className="flex items-center gap-2">
                          <Plane className="text-white opacity-80" />
                          <span className="font-black italic tracking-tighter text-white">LIFETIME</span>
                      </div>
                      <span className="font-mono text-xs opacity-70">LF-882910</span>
                  </div>
                  <div>
                      <span className="text-xs uppercase tracking-widest opacity-60">Available Points</span>
                      <div className="text-4xl font-black tracking-tight drop-shadow-md">{pointsBalance.toLocaleString()}</div>
                  </div>
              </div>
          </div>
          ...
          <div className="px-6 -mt-16 relative z-10">
              <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-6">
                  <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                      <Users size={18} className="text-sky-600" /> My Travelers
                  </h3>
                  <div className="space-y-3">
                      {travelers.map(t => (
                          <div 
                            key={t.id} 
                            onClick={() => setEditingTravelerId(t.id)}
                            className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 cursor-pointer border border-gray-100 transition-colors"
                          >
                              <div className="flex items-center gap-3">
                                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${t.role === 'Child' ? 'bg-orange-100 text-orange-600' : 'bg-sky-100 text-sky-600'}`}>
                                      {t.name[0]}
                                  </div>
                                  <div>
                                      <div className="font-bold text-sm text-slate-900">{t.name}</div>
                                      <div className="text-xs text-gray-500 flex items-center gap-2">
                                          <span>{t.seatPref}</span>
                                          <span className="w-1 h-1 bg-gray-300 rounded-full" />
                                          <span>{t.mealPref}</span>
                                      </div>
                                  </div>
                              </div>
                      ...
                      <button className="w-full py-3 border border-dashed border-gray-300 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-50 flex items-center justify-center gap-2">
                          <Plus size={14} /> Add New Traveler
                      </button>
                  </div>
              </div>

              <div className="space-y-4">
                  <h3 className="font-bold text-sm text-gray-400 uppercase tracking-widest px-2">Points History</h3>
                  {[
                      { desc: "Flight to Costa Rica", pts: "+4,200", date: "Mar 12" },
                      { desc: "Amex Point Transfer", pts: "+10,000", date: "Feb 28" },
                      { desc: "Juno Store Partner", pts: "+550", date: "Feb 15" }
                  ].map((item, i) => (
                      <div key={i} className="flex justify-between items-center px-4 py-3 bg-white rounded-xl border border-gray-100">
                          <div>
                              <div className="font-bold text-sm text-slate-900">{item.desc}</div>
                              <div className="text-xs text-gray-500">{item.date}</div>
                          </div>
                          <div className="font-mono font-bold text-green-600 text-sm">{item.pts}</div>
                      </div>
                  ))}
              </div>
          </div>
  ...
  const renderShareSheet = () => (
      ...
      >
          ...
          >
              <div className="flex justify-between items-center mb-6">
                  <h3 className="font-bold text-xl text-slate-900">Share Trip</h3>
                  <button onClick={() => setShowShareSheet(false)} className="p-2 bg-gray-100 rounded-full"><X size={20} /></button>
              </div>

              <div className="space-y-4">
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Recent Contacts</h4>
                  <div className="space-y-2">
                      {/* Using travelers for demo sharing targets */}
                      {travelers.filter(t => t.id !== 'eloise' && t.role === 'Adult').map(t => (
                          <div 
                              key={t.id}
                              onClick={() => handleShareToContact(t.id, t.name)}
                          ...
                          >
                              ...
                              <div className="bg-slate-900 text-white px-4 py-1.5 rounded-full text-xs font-bold">Send</div>
                          </div>
                      ))}
                      {/* Add a generic contact if list is empty or small */}
                      <div 
                          onClick={() => handleShareToContact('mom', 'Mom')}
                          className="flex items-center gap-4 p-3 hover:bg-gray-50 rounded-xl cursor-pointer transition-colors"
                      >
                          ...
                          <div className="flex-1">
                              <div className="font-bold text-slate-900">Mom</div>
                              <div className="text-xs text-gray-500">iMessage</div>
                          </div>
                          <div className="bg-slate-900 text-white px-4 py-1.5 rounded-full text-xs font-bold">Send</div>
                      </div>
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
          ...
          {editingTravelerId && renderEditTraveler()}
      </AnimatePresence>

      {/* Header */}
      <div className="pt-12 px-6 pb-4 bg-white shadow-sm z-20 flex justify-between items-center">
          <div className="flex items-center gap-2">
              {viewState !== 'SEARCH' && activeTab === 'BOOKING' ? (
                  <button onClick={() => {
                      if (viewState === 'RESULTS') setViewState('SEARCH');
                  ...
                  <button onClick={onClose} className="p-1 -ml-2 rounded-full hover:bg-gray-100">
                      <ChevronLeft size={24} className="text-sky-600" />
                  </button>
              )}
              
              <div className="flex flex-col">
                  <h1 className="font-black text-xl text-sky-600 italic tracking-tighter leading-none">Lifetime Flights</h1>
                              ...
                              <div className="pt-2">
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                      {travelers.map((member) => (
                                          <button
                                              key={member.id}
                                              onClick={() => togglePassenger(member.name.split(' ')[0])}
                                              className={`px-3 py-1.5 rounded-full text-xs font-bold border flex items-center gap-1 transition-all ${passengers.includes(member.name.split(' ')[0]) ? 'bg-sky-600 text-white border-sky-600 shadow-sm' : 'bg-white text-gray-500 border-gray-200'}`}
                                          >
                                              {passengers.includes(member.name.split(' ')[0]) && <Check size={12} strokeWidth={3} />}
                                              {member.name.split(' ')[0]}
                                          </button>
                                      ))}
                                  </div>
                          ...
                          <div className="flex items-center justify-between mb-4">
                              <div className="flex items-center gap-2">
                                  ...
                                  <div className="bg-purple-100 text-purple-900 px-3 py-2 rounded-xl text-xs font-medium max-w-[200px]">
                                      <strong>Silas:</strong> Select a passenger, then tap a seat.
                                  </div>
                              </div>
                          </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[83px] bg-white border-t border-gray-200 flex justify-around items-start pt-3 pb-8 z-30">
          ...
          <button onClick={() => setActiveTab('PROFILE')} className={`flex flex-col items-center gap-1 ${activeTab === 'PROFILE' ? 'text-sky-600' : 'text-gray-400'}`}>
              <User size={24} />
