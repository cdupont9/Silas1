...
export const MessagesApp: React.FC<MessagesAppProps> = ({ onClose, contacts, setContacts, conversations, setConversations, onSendMessage }) => {
  ...
  // Initialize with robust conversations if empty
  useEffect(() => {
      // Check if we need to hydrate conversations with more data
      const danielConv = conversations.find(c => c.contactId === 'daniel');
      if (danielConv && danielConv.messages.length <= 7) {
          const extraMessages: Message[] = [
              { id: 'd-hist-1', text: 'Are you bringing wine for tonight?', isMe: true, timestamp: new Date(Date.now() - 3600000 * 24 * 2) },
              { id: 'd-hist-2', text: 'Already got it. A nice Cab.', isMe: false, timestamp: new Date(Date.now() - 3600000 * 24 * 2 + 60000) },
              { id: 'd-hist-3', text: 'You’re the best. Also, did you call the plumber?', isMe: true, timestamp: new Date(Date.now() - 3600000 * 24 * 2 + 120000) },
              { id: 'd-hist-4', text: 'Yes, he’s coming Tuesday.', isMe: false, timestamp: new Date(Date.now() - 3600000 * 24 * 2 + 180000) },
          ];
          // Prepend history
          setConversations(prev => prev.map(c => c.contactId === 'daniel' ? { ...c, messages: [...extraMessages, ...c.messages] } : c));
      }

      const momConv = conversations.find(c => c.contactId === 'c2'); // Mom
      if (momConv && momConv.messages.length <= 5) {
          const extraMomMessages: Message[] = [
              { id: 'm-hist-1', text: 'Saw a cute dress at Macy’s today, reminded me of you.', isMe: false, timestamp: new Date(Date.now() - 3600000 * 48) },
              { id: 'm-hist-2', text: 'Send a pic!', isMe: true, timestamp: new Date(Date.now() - 3600000 * 48 + 50000) },
              { id: 'm-hist-3', text: 'I didn’t take one, sorry! Next time.', isMe: false, timestamp: new Date(Date.now() - 3600000 * 48 + 100000) },
          ];
          setConversations(prev => prev.map(c => c.contactId === 'c2' ? { ...c, messages: [...extraMomMessages, ...c.messages] } : c));
      }
  }, []);
