...
import { CloudSun, CloudRain, Sun, Wind, Droplets, MapPin, List, Menu, Bot, Sparkles, Shirt, Calendar, ArrowRight, Cloud, Moon, Search, X, ShoppingBag, Tag, ChevronDown, Layers, Receipt, Mail, CheckCircle, CalendarDays, Clock, RefreshCw } from 'lucide-react';
...
export const WeatherApp: React.FC<WeatherAppProps> = ({ onClose, calendarEvents }) => {
  ...
  // Simulation Date
  const SIMULATED_TODAY = "2026-01-01";
  const todaysEvents = calendarEvents.filter(e => e.date === SIMULATED_TODAY);
  ...
  const getAdvice = async (currentWeather: WeatherData) => {
      ...
      const scheduleText = todaysEvents.map(e => `${e.time}: ${e.title} at ${e.location}`).join(", ");
      ...
      const prompt = `
          ...
          - Full Schedule Today: ${scheduleText || "No events scheduled today"}.
  ...
  const getOutfitRecommendations = async (currentWeather: WeatherData) => {
      // If we already have options and aren't forcing a refresh, skip
      if (outfitOptions && activeTab === 'OUTFIT' && !isLoadingOutfit) return;
                    ...
                    <div className="p-6 flex-1 overflow-y-auto">
                        ...
                        {todaysEvents.length > 0 ? (
                            ...
                            <div className="text-center text-gray-500 mt-20">
                                No events scheduled for today ({SIMULATED_TODAY}).
                            </div>
                        )}
                    </div>
                </motion.div>
            ...
            {activeTab === 'OUTFIT' && (
                ...
                >
                    ...
                    <div className="pt-14 px-6 pb-2 flex justify-between items-center z-10">
                        ...
                        <button 
                            onClick={() => getOutfitRecommendations(weather)}
                        ...
                        >
                            <RefreshCw size={20} className={`text-white ${isLoadingOutfit ? 'animate-spin' : ''}`} />
                        </button>
                    </div>
                                ...
                                {outfitOptions?.map((opt, i) => (
                                    ...
                                    >
                                        ...
                                        <div className="h-64 w-full bg-gray-800 relative overflow-hidden">
                                            {!imageLoadErrors[i] && opt.imageUrl ? (
                                                <img 
                                                    src={opt.imageUrl} 
                                                ...
                                                <div className="w-full h-full flex flex-col items-center justify-center bg-gray-800 p-4 text-center">
                                                    <Shirt className="text-white/20 mb-2" size={40} />
                                                    <span className="text-xs text-white/40">Visual Unavailable</span>
                                                </div>
                                            )}
