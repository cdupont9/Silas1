...
export interface SilasActivity {
    id: string;
    action: string;
    detail: string;
    timestamp: Date;
    type: 'VOICE' | 'AUTO' | 'SUGGESTION';
    iconName?: string; // 'Message', 'Cart', 'Search', etc.
}

export interface SilasAction {
    type: 'OPEN_APP' | 'SEND_MESSAGE' | 'ADD_CART' | 'SEARCH' | 'PLAY_MUSIC' | 'WEATHER' | 'FLIGHT_INFO' | 'BOOK_FLIGHT' | 'NONE';
    payload?: any;
    reply: string; // Spoken response
    followUpQuestion?: string; // e.g. "Do you want an outfit suggestion?"
}